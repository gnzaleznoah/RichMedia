...
//Variables for Original Audio
const changeAudioBtn = document.querySelector('#original-audio');
const audioOrig = document.querySelector('.orig-aud');
...
function togglePlay(){
    ...
    video[vidState]();
    const audState = audioOrig.paused ? 'play' : 'pause';
    audioOrig[audState]();
    const backMusic = backgroundMusic.paused ? 'play' : 'pause';
...
function skip(){
    ...
    backgroundMusic.currentTime += parseFloat(this.dataset.skip);
}
//Volume
function rangeUpdate(e){
    //video[this.name] = this.value;
    video.volume = e.currentTarget.value /100;
    audioOrig.volume = e.currentTarget.value /100;
...
function scrub(e){
    ...
    backgroundMusic.currentTime = scrubTime;
    audioOrig.currentTime = scrubTime;
}
function fullScreen(){
...
function foleyUpdate(){
    audioOrig.classList.toggle('muted');
    if(video.muted === true){
        video.muted = false;
        audioOrig.muted = true;
        ...
        audioOrig.muted = false;
        audioOrig.volume = 1;
        changeAudioBtn.innerHTML = 'FOLEY is Off';   
    }
}
function backVolume(){
    if(backgroundMusic.muted === true){
        ...
        //backgroundMusic.volume = rangeUpdate();
        backButn.innerHTML='Background  is ON';
...
function func1(){
    addClass('.chill-coffee', '.opacity-change')
}

// //Toggle Original to Foley
changeAudioBtn.addEventListener('click', foleyUpdate);
